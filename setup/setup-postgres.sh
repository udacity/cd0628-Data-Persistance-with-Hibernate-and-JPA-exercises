...
psql -U postgres -tAc "SELECT 1 FROM pg_roles WHERE rolname='${ROLE_NAME}'" | grep -q 1 \
  || psql -U postgres -c "CREATE USER ${ROLE_NAME} WITH PASSWORD '${ROLE_PASSWORD}';"

# Create the database only if missing
psql -U postgres -tAc "SELECT 1 FROM pg_database WHERE datname='${DB_NAME}'" | grep -q 1 \
  || psql -U postgres -c "CREATE DATABASE ${DB_NAME} OWNER ${ROLE_NAME};"

# Ensure privileges (no-op if already granted)
psql -U postgres -c "GRANT ALL PRIVILEGES ON DATABASE ${DB_NAME} TO ${ROLE_NAME};" > /dev/null
