...
public class OrderSearchService {
    ...
    /**
     * Dynamic search: build a predicate list from non-null filters,
     * then combine with AND. Skipped filters mean no WHERE clause for
     * that field, so all-null criteria returns the full table.
     */
    public List<Order> searchOrders(OrderSearchCriteria criteria) {
        ...
        if (criteria.fromDate() != null && criteria.toDate() != null) {
            predicates.add(cb.between(root.get("placedAt"), criteria.fromDate(), criteria.toDate()));
        }
